...
ranger_hbase_plugin=ranger-$ranger_download_version-hbase-plugin
...
#Setup
sudo rm -rf $installpath/$ranger_hbase_plugin
mkdir -p $installpath/hbase/lib
mkdir -p $installpath/hbase/conf
cd $installpath
mkdir $ranger_hbase_plugin
...
sudo ln -sfn /etc/hbase/conf $installpath/hbase/conf
sudo ln -sfn /usr/lib/hbase $installpath/hbase/lib
...
sudo sed -i "s|COMPONENT_INSTALL_DIR_NAME=.*|COMPONENT_INSTALL_DIR_NAME=/usr/lib/hbase|g" install.properties
...
sed -i 's|jceks://file|localjceks://file|g' enable-hbase-plugin.sh
