...
#Keeping this as a variable that can be modified
cloudwatch_audits=true
cloudwatch_log_group="/aws/emr/rangeraudit/"

#When available we'll include 3.0
case $ranger_version in

  2.0)
      echo -n "Installing Ranger Version 2.4.0"
          ranger_download_version=2.4.0
          cloudwatch_audits=true
      ;;

  *)
    echo -n "Installing Default Ranger Version 2.4.0"
    ranger_download_version=2.4.0
    cloudwatch_audits=true
    ;;
esac
...
if [ "$cloudwatch_audits" = true ] ; then
    echo 'Configuring Cloudwatch audits'
    sudo sed -i "s|audit_store=.*|audit_store=cloudwatch|g" install.properties
    sudo sed -i "s|audit_cloudwatch_region=.*|audit_cloudwatch_region=$default_region|g" install.properties
    sudo sed -i "s|audit_cloudwatch_log_group=.*|audit_cloudwatch_log_group=$cloudwatch_log_group|g" install.properties
    sudo sed -i "s|audit_cloudwatch_log_stream_prefix=.*|audit_cloudwatch_log_stream_prefix=|g" install.properties
else
  echo 'Configuring SOLR audits'
...
sudo cp $installpath/$ranger_admin_server/ews/webapp/WEB-INF/lib/htrace-core* $installpath/$ranger_admin_server/cred/lib
sudo cp $installpath/$ranger_admin_server/ews/webapp/WEB-INF/lib/commons-configuration* $installpath/$ranger_admin_server/cred/lib

## Install the spark ranger plugin
sudo mkdir -p $installpath/$ranger_admin_server/ews/webapp/WEB-INF/classes/ranger-plugins/amazon-emr-spark
cp -r $installpath/$ranger_admin_server/ews/webapp/WEB-INF/classes/ranger-plugins/hive/* $installpath/$ranger_admin_server/ews/webapp/WEB-INF/classes/ranger-plugins/amazon-emr-spark/

chmod +x setup.sh
./setup.sh

#CHECKTHIS - FIX FOR Unable to get the Credential Provider from the Configuration when launching the server
sudo sed -i "s|.*rangeradmin.jceks.*|<value>localjceks://file//usr/lib/ranger/$ranger_admin_server/ews/webapp/WEB-INF/classes/conf/.jceks/rangeradmin.jceks</value>|g" $installpath/$ranger_admin_server/ews/webapp/WEB-INF/classes/conf/ranger-admin-default-site.xml
...
sudo cp $installpath/$ranger_admin_server/ews/webapp/WEB-INF/lib/commons-configuration* $installpath/$ranger_user_sync/lib/
...
sudo mkdir -p $installpath/logs/admin/
sudo ln -sfn $installpath/$ranger_admin_server/ews/logs $installpath/logs/admin/logs
...
log4j.additivity.org.apache.ranger.rest.ServiceREST=false" >> $installpath/$ranger_admin_server/ews/webapp/WEB-INF/log4j.properties
sudo ln -s $installpath/$ranger_admin_server/ews/webapp/WEB-INF/classes/ranger-plugins/hive/ranger-hive-plugin-$ranger_download_version* $installpath/$ranger_admin_server/ews/webapp/WEB-INF/lib/
sudo ln -s $installpath/$ranger_admin_server/ews/webapp/WEB-INF/classes/ranger-plugins/hdfs/ranger-hdfs-plugin-$ranger_download_version* $installpath/$ranger_admin_server/ews/webapp/WEB-INF/lib/

# Setup the Spark Ranger plugin definition
sudo rm -rf $installpath/$ranger_admin_server/ews/webapp/WEB-INF/classes/ranger-plugins/amazon-emr-spark
sudo mkdir -p $installpath/$ranger_admin_server/ews/webapp/WEB-INF/classes/ranger-plugins/amazon-emr-spark
...
sudo mv /tmp/ranger-spark-plugin-2.x.jar $installpath/$ranger_admin_server/ews/webapp/WEB-INF/classes/ranger-plugins/amazon-emr-spark/

# Setup the EMRFS Ranger plugin definition
sudo rm -rf $installpath/$ranger_admin_server/ews/webapp/WEB-INF/classes/ranger-plugins/amazon-emr-emrfs
sudo mkdir -p $installpath/$ranger_admin_server/ews/webapp/WEB-INF/classes/ranger-plugins/amazon-emr-emrfs
...
sudo mv /tmp/ranger-emrfs-s3-plugin-2.x.jar $installpath/$ranger_admin_server/ews/webapp/WEB-INF/classes/ranger-plugins/amazon-emr-emrfs/


#CHECKTHIS - wrong path
sudo cp $installpath/$ranger_admin_server/ews/webapp/WEB-INF/classes/ranger-plugins/hive/* $installpath/$ranger_admin_server/ews/webapp/WEB-INF/lib/ || true
...
password=$(echo -n "${ranger_admin_password}{admin}" | md5sum | awk '{print $1}')

_generateSQLUpdateAdminPassword()
{
    touch ~/generate_admin_password.sql
    cat >~/generate_admin_password.sql <<EOF
update x_portal_user set password = '${password}' where login_id = 'admin';
exit
EOF
}
...
{
    HOSTNAMEI=`hostname -I`
    ${MYSQL} -h ${db_host_name} -u ${DB_ROOT_USERNAME} -p${db_root_password} --database=${RDS_RANGER_SCHEMA_DBNAME} < ~/generate_admin_password.sql
    echo $?
}



if [ "$ranger_admin_password" != "admin" ] ; then
  echo "Changing admin user password"
  _generateSQLUpdateAdminPassword
  _updateAdminPassword
fi

## Prepare for trino setup
curl -iv --insecure -u admin:${ranger_admin_password} -X DELETE -k $HTTP_URL/service/public/v2/api/servicedef/name/presto
curl -iv --insecure -u admin:${ranger_admin_password} -X DELETE -k $HTTP_URL/service/public/v2/api/servicedef/name/trino

sudo mkdir -p $installpath
sudo chmod -R 777 $installpath
cd $installpath
aws s3 cp $s3bucket/${project_version}/inputdata/service-definition/$ranger_service_def_ver/ . --recursive --exclude "*" --include "*.json" --region $default_region
for i in `find . -name "ranger-servicedef-*.json" -type f`; do
    file_name=`echo "$i" | cut -c 3-`
    echo "$file_name"
    curl -iv --insecure -u admin:${ranger_admin_password} -X POST -d @$file_name -H "Accept: application/json" -H "Content-Type: application/json" -k $HTTP_URL/service/public/v2/api/servicedef
done

aws s3 cp $s3bucket/${project_version}/inputdata/ranger-users/ . --recursive --exclude "*" --include "*.json" --region $default_region
for i in `find . -name "ranger-*-user.json" -type f`; do
    file_name=`echo "$i" | cut -c 3-`
    echo "$file_name"
    curl -iv --insecure -u admin:${ranger_admin_password} -X POST -d @$file_name -H "Accept: application/json" -H "Content-Type: application/json" -k $HTTP_URL/service/xusers/secure/users
done
